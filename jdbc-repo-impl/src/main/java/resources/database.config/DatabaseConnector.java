...
public class DatabaseConnector implements Repository, Repository.Provider {
    ...
    public DatabaseConnector() {
        try {
            this.conn = DriverManager.getConnection(DB_URL, "postgres", "postgres");
            setup();
    ...
    public static DatabaseConnector instance(String username, String password) {
        try {
            var conn = DriverManager.getConnection(DB_URL, username, password);
    ...
    public void save(Orderitem orderitem) throws SQLException {
        if (orderitem == null || orderitem.id() == null || orderitem.id().value() == null) {
            throw new IllegalArgumentException("Orderitem or orderitem ID cannot be null");
        }

        var tableName = "Orderitem";
        if (!exists(tableName, orderitem.id().value())) {
            var query = INSERT_INTO("\"" + tableName + "\"")
                    .VALUE("id", orderitem.id().value())
                    .VALUE("status", orderitem.status().name()) // Ensure enum is properly converted to String
            ...
            var query = UPDATE("\"" + tableName + "\"")
                    ...
                    .SET("status", orderitem.status().name()) // Ensure enum is properly converted to String
    ...
    public void save(Order order) throws SQLException {
        if (order == null || order.id() == null || order.id().value() == null) {
            throw new IllegalArgumentException("Order or order ID cannot be null");
        }

        var tableName = "Order";
        if (!exists(tableName, order.id().value())) {
            var query = INSERT_INTO("\"" + tableName + "\"")
    ...
    public Optional<Order> getOrder(Id<Order> id) {
        ...
        try (var result = conn.createStatement().executeQuery(query)) {
            return result.next() ?
    ...
    public List<Compartment> get(Compartment.Filter filter) {
        ...
        filter.row().ifPresent(ref -> query.WHERE("row", ref));
        filter.column().ifPresent(ref -> query.WHERE("column_position", ref));
    ...
    public List<Order> get(Order.Filter filter) {
        ...
        filter.supplier().ifPresent(ref -> query.WHERE("supplier_id", ref.id().value()));
    ...
    public List<Stock> get(Stock.Filter filter) {
        ...
        filter.compartment().ifPresent(ref -> query.WHERE("compartment_id", ref.id().value()));

        filter.amount().ifPresent(ref -> query.WHERE("amount", ref.id().value()));

        filter.id().ifPresent(id -> query.WHERE("medication_id", id.value()));
    ...
    public List<Orderitem> get(Orderitem.Filter filter) {
        ...
        filter.order().ifPresent(ref -> query.WHERE("order_id", ref.id().value()));

        filter.status().ifPresent(status -> query.WHERE("status", status.name()));
        filter.medication().ifPresent(ref -> query.WHERE("medication_id", ref.id().value()));
    ...
    public List<Stock> getMedicationsWithLowStock(int threshold) {
        String query = "SELECT s.id, s.amount, s.expirationdate, s.medication_id, s.compartment_id " +
                "FROM \"Stock\" s " +
                "WHERE s.amount < ?";

        List<Stock> stocksWithLowStock = new ArrayList<>();
            ...
            try (var rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    Stock stock = new Stock(
                            ...
                            Reference.to(rs.getString("medication_id")),
                            Reference.to(rs.getString("compartment_id"))
                    );
                    stocksWithLowStock.add(stock);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        return stocksWithLowStock;
    }



    public List<Stock> getMedicationsWithExpiredStock() {
        String query = "SELECT s.id, s.amount, s.expirationdate, s.medication_id, s.compartment_id " +
                "FROM \"Stock\" s " +
                "WHERE s.expirationdate < CURRENT_DATE";

        List<Stock> stocksWithExpiredStock = new ArrayList<>();
        try (var stmt = conn.createStatement();
            ...
            while (rs.next()) {
                ...
                stocksWithExpiredStock.add(stock);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }

        return stocksWithExpiredStock;
    }
